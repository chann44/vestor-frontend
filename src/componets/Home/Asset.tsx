...
const AssetComponet = ({balance, contract_name, quote, contract_address, logo_url, quote_rate}: IAssetInfo) => {
  return (
<div className="grid grid-cols-5 p-5 bg-secondaryDark items-center rounded-x my-2">
      ...
      <div className="col-start-3 col-span-1 flex justify-center  ">
        <p className="text-sm">{ethers.utils.formatEther(balance).toString().slice(0, 8)}</p>
      </div>
      <div className="col-start-4 col-span-1  flex justify-center ">
      ...
      <div className="col-start-5 col-span-1 flex justify-center  ">
        <p className="text-sm text-text-faded">{contract_address?.slice(0, 5)}</p>
      </div>
    </div>
  );
};
    ...
    <div className=" bg-primaryDark w-full p-5 rounded-xl">
      ...
      {token &&
        token?.map((asset: IAssetInfo) => {
          return <AssetComponet quote_rate={asset.quote_rate} balance={asset.balance} contract_address={asset.contract_address} contract_decimals={asset.contract_decimals} contract_name={asset.contract_name} logo_url={asset.logo_url} last_transferred_at={asset.last_transferred_at} quote={asset.quote} key={asset.contract_address} />;
        })}
    </div>
  );
};
